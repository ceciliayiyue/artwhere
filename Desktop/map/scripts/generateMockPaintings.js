...
function safeGetCoordinates(obj) {
  if (!obj) return null;
  // obj may be { coordinates: [lat, lng], name }
  if (Array.isArray(obj) && obj.length >= 2 && typeof obj[0] === 'number') {
    // sometimes `location` is an array of locations
...
try {
  ...
  const mockPaintings = sample.map((item, idx) => {
    ...
    // Ensure there is at least one round; if none, insert a fallback using country_of_origin coords if present
    if (!rounds.length) {
